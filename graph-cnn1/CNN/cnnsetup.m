...
% 该函数你用于初始化CNN的参数。
% 设置各层的mapsize大小，初始化卷积层的卷积核、bias、尾部单层感知机的参数设置
% bias统一初始化为0
% 权重设置为：random(-1,1)/sqrt（6/（输入神经元数量+输出神经元数量）)
% 对于卷积核权重，输入输出为fan_in, fan_out
% fan_in是对每一个输出map,与输入maps相连的参数个数
% fan_out是对每一个输入map,与输出maps相连的参数个数
% net.layers{l}.k{i}{j} = (rand(net.layers{l}.kernelsize) - 0.5) * 2 * sqrt(6 / (fan_in + fan_out));
    ...
    % net.layers{i}.type = s 表示这是一个子采样层，= i 表示是输入层，= c 表示是卷积层
    for l = 1 : numel(net.layers)   %  layer
        if strcmp(net.layers{l}.type, 's') % 如果这层是子采样层
            ...
            % inputmap就是上一层有多少张特征图，pooling不改变这个值
            for j = 1 : inputmaps 
        ...
        if strcmp(net.layers{l}.type, 'c') % 如果这层是卷积层
            ...
            % fan_out是对于上一层的一张特征map，所对应的需要学习的参数个数。
            fan_out = net.layers{l}.outputmaps * net.layers{l}.kernelsize ^ 2;
            for j = 1 : net.layers{l}.outputmaps  %  output map
                % fan_in是对于每一个当前层特征图，有多少个参数链到前层
    ...
    % ffW 输出层前一层 与 输出层 连接的权值，这两层之间是全连接的（注意行列）
