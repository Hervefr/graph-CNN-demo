...
% 前向传播
    ...
    % CNN层数
    n = numel(net.layers);
    %  网络的第一层就是输入，a是输入map，是一个[28, 28, 50]的矩阵
    net.layers{1}.a{1} = x{1};
    net.layers{1}.a{2} = x{2};
    % 输入层只有一个特征map，也就是原始的输入图像
    inputmaps = 2;
    ...
    for l = 2 : n   %  for each layer
        if strcmp(net.layers{l}.type, 'c')
            ...
            for j = 1 : net.layers{l}.outputmaps   %  for each output map
                ...
                % 所以，这里的z保存的就是该层中一个输出map了
                z = zeros(size(net.layers{l - 1}.a{1}) - [net.layers{l}.kernelsize - 1 net.layers{l}.kernelsize - 1 0]); % 后面有一个0的原因是maps的最后一维是样本数量
