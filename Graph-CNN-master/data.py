...
class Data(object):
    ...
    def __init__(self, path, ratio = 0.9, batch_size = 1):
        ...
        self.ratio = ratio # 用作训练集比例，这里90%的数据集用于训练
    ...
    def read_data(self, path):
		...
		# 得到每一个graph的节点个数
        node_count = [] 
        for gidx, graph in self.graph_set.items(): #gidx是图索引号
        ...
        self.len, self.classes = len(self.graph_set), len(Counter(self.labels)) #self.len 图的数量, self.classes 分类数
        
        self.size = pow(2, int(np.log2(self.median))) #number of vertices to keep from each graph 
        self.shuffled          = np.arange(int(self.len*self.ratio))
        np.random.shuffle(self.shuffled) # 打乱顺序
    ...
    def load_data(self, path):
        f = open(path, "rb")
        data = pickle.load(f, encoding='latin1') # pickle.load(file) 从file中读取一个字符串，并将它重构为原来的python对象
        graph_data = data["graph"]
        labels = data["labels"]
        labels  = np.array(labels, dtype = np.int) # np.int表示数据类型是整数，np.array函数将Python对象生成数组
